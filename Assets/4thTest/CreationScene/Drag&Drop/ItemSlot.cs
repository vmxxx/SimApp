...
public class ItemSlot : MonoBehaviour, IDropHandler
{
    ...
    public void OnDrop(PointerEventData eventData)
    {
        ...
        if (eventData.pointerDrag != null)
        {
            ...
            if (indexX == 0)
            {
                oppositeCell = payoffMatrixPanel.transform.Find("TableColumn_" + indexY).Find("TableCell_0_" + indexY).gameObject;
            }
            else
            {
                oppositeCell = payoffMatrixPanel.transform.Find("TableColumn_0").Find("TableCell_" + indexX + "_0").gameObject;
            }
